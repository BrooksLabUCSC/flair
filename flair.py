...
def collapse(genomic_range='', corrected_reads=''):
	...
	if args.p:
		...
		subprocess.call([sys.executable, path+'bin/psl_reads_from_bed.py', args.temp_dir+tempfile_name+'promoter_intersect.bed', \
			args.q, precollapse])
		intermediate += [args.o+'tss.bed', args.o+'promoter_supported'+ext]
...
# 	mode = 'collapse-range'
# elif len(sys.argv) > 1 and (sys.argv[1].lower() == 'collapse-range' or sys.argv[1] == '3.5'):
# 	mode = 'collapse-new'
...
# 	required = parser.add_argument_group('required named arguments')
# 	required.add_argument('-r', '--reads', action='store', dest='r', nargs='+', \
# 		type=str, required=True, help='FastA/FastQ files of raw reads; if --range is provided, then' + \
# 		 'a bam file of the aligned reads')
# 	required.add_argument('-q', '--query', type=str, default='', required=True, \
# 		action='store', dest='q', help='bed or psl file of aligned/corrected reads')
# 	required.add_argument('-g', '--genome', action='store', dest='g', \
# 		type=str, required=True, help='FastA of reference genome')
...
# 	# partition the bed file into independent regions
# 	subprocess.call([bedPartition, args.q, args.o+'.ranges.bed'])
# 	ranges = []
# 	for line in open(args.o+'.ranges.bed'):
# 		line = line.rstrip().split('\t')
# 		ranges.add(line[0]+':'+line[1]+'-'+line[2])
		
# 	tempfile_name = tempfile.NamedTemporaryFile().name
# 	unique = tempfile_name[tempfile_name.rfind('/'):]

# 	# index the bed file 
# 	subprocess.call(['sort','-k1,1', '-k2,2n', args.q], stdout=open(args.q+'.sorted.bed', 'w'))
# 	subprocess.call(['bgzip', args.q+'.sorted.bed'])
# 	if subprocess.call(['tabix', '--preset', 'bed', '--zero-based', args.q+'.sorted.bed.gz']):
# 		sys.exit(1)

# 	# call collapse-range on all the intervals using a tabix- bed file
# 	f = [sys.executable, 'flair.py', 'collapse', '--bam', args.bam, '--range', ranges[0], '-o', \
# 	unique+'.'+ranges[0] ]

# 	subprocess.call(['cat', unique+'*.bed'], stdout=open(args.o+'.isoforms.bed', 'w'))
# 	subprocess.call(['cat', unique+'*.fa'], stdout=open(args.o+'.isoforms.fa', 'w'))
# 	subprocess.call(['rm', unique+'*'])
...
# 		note: only works if the quantification method is not using salmon (default: not specified)''')
# 	parser.add_argument('--range', default='', action='store', dest='range', \
# 		help='''interval for which to collapse isoforms for, formatted chromosome:coord1-coord2; if a range is
# 		specified, then the aligned reads bam must be specified with -r 
# 		and the query must be a sorted, bgzip-ed bed file''')	
# 	parser.add_argument('--salmon', type=str, action='store', dest='salmon', \
# 		default='', help='Path to salmon executable, specify if salmon quantification is desired')
# 	parser.add_argument('--temp_dir', default='', action='store', dest='temp_dir', \
...
# 		if subprocess.call([args.sam, 'view', '-h', args.r[0], args.range], \
# 			stdout=open(args.temp_dir+args.range+'.bam', 'w')):  # subset bam
# 			sys.exit(1)
# 		subprocess.call([args.sam, 'fasta', args.temp_dir+args.range+'.bam'], \
# 			stdout=open(args.temp_dir+args.range+'.fasta', 'w'))  # subset reads
# 		args.r = [args.temp_dir+args.range+'.fasta']
# 		chrom  = args.range[:args.range.find(':')]
# 		coord1 = args.range[args.range.find(':')+1:args.range.find('-')]
# 		coord2 = args.range[args.range.find('-')+1:]
# 		precollapse = args.temp_dir+args.range+'.bed'  # name of subsetted query file
# 		coordfile = open(args.temp_dir+args.range+'.range.bed', 'wt')
# 		coordfile.write('\t'.join([chrom, coord1, coord2]))
# 		coordfile.close()
# 		if subprocess.call(['tabix', '-R', args.temp_dir+args.range+'.range.bed', args.q], \
# 			stdout=open(precollapse, 'w')):
# 			sys.stderr.write('Query file needs to be a sorted, bgzip-ed, tabix-indexed bed file if range is specified\n')
# 			sys.exit(1)
...
# 		if subprocess.call([sys.executable, path+'bin/pull_starts.py', args.q, args.temp_dir+'tss.bed']):
# 			sys.exit(1)
# 		if subprocess.call([args.b, 'intersect', '-a', args.temp_dir+'tss.bed', '-b', args.p], \
# 			stdout=open(args.temp_dir+'promoter_intersect.bed', 'w')):
# 			sys.exit(1)
# 		precollapse = args.o+'.promotersupported.'+suffix  # filename of promoter-supported, corrected reads
# 		subprocess.call([sys.executable, path+'bin/psl_reads_from_bed.py', args.temp_dir+'promoter_intersect.bed', \
# 			args.q, precollapse])
# 		intermediate += [args.o+'.tss.bed', args.o+'.promotersupported.'+suffix]
...
# 	alignout = args.temp_dir + args.o.replace('/', '.') +'.firstpass'
# 	try:  # reassign reads to first-pass isoforms
# 		if subprocess.call([args.m, '-a', '-t', args.t, '-N', '4', args.o+'.firstpass.fa'] + args.r, \
...
# 	if args.salmon:  # use salmon to count supporting reads for each first-pass isoform
# 		if subprocess.call([args.sam, 'view', '-F', '4', '-h', '-S', alignout+'.sam'], \
# 			stdout=open(alignout+'.mapped.sam', 'w')):
# 			sys.exit(1)
# 		subprocess.call(['mv', alignout+'.mapped.sam', alignout+'.sam'])
...
# 		subprocess.call(['rm', '-rf', args.temp_dir] + align_files + intermediate)
