...
def check_splicesites(coveredpos, exonpos):
	currpos = 0
	...
	for blen, btype in cigarblocks:
		...
		if btype == 'S' or btype == 'H':
			...
			if args.stringent or args.check_splice:
				...
				tendpos += blen
			if blen > large_indel_tolerance: return True, None, None, None, None #
		elif btype == 'I':
			if args.stringent or args.check_splice:
				coveredpos[-1] += blen
			if blen > large_indel_tolerance: return True, None, None, None, None
	return False, coveredpos, queryclipping, blockstarts, blocksizes
...
def check_stringentandsplice(args, transcripttoexons, tname, coveredpos, tlen, blockstarts, blocksizes):
	...
	if args.stringent or args.check_splice:
		...
		passessplice = check_splicesites(coveredpos, exoninfo) if args.check_splice else True
...
def getbesttranscript(tinfo, args, transcripttoexons):
	...
	for tname in tinfo:
		...
		indel_detected, coveredpos, queryclipping, blockstarts, blocksizes  = process_cigar(args, matchvals, thist.cigar, thist.startpos)

		if not indel_detected:
			if check_stringentandsplice(args, transcripttoexons, thist.name, coveredpos, thist.tlen, blockstarts, blocksizes):
