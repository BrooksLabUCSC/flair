...
def parse_args():
    ...
    parser.add_argument("--is_flair_bed", action='store_true', help='specify if input is a bed12+ file generated by flair, '
                                                                    'will add productivity to flair bed productivity column')
...
def predict_productivity(gtf, genome, bed, output, is_flair_bed):  # noqa: C901 - FIXME: reduce complexity
    ...
    isoformObjs = getSeqs(shortbedname, genome)
    isoformObjs = predict(shortbedname, starts, isoformObjs, nmdexcep)
    ...
    for bed_rec in BedReader(bed, fixScores=True, bedClass=my_bed_class):
        ...
        if is_flair_bed:
            bed_rec.productivity = pro
        else:
            bed_rec.extraCols = [pro]
    ...
    bed_to_gtf(output + '.bed', output + '.gtf', is_flair_bed=is_flair_bed)


def main():
    args = parse_args()
    predict_productivity(args.gtf, args.genome_fasta, args.input_isoforms, args.output, args.is_flair_bed)
