...
def read_vars_to_genome_pos_counts(tempfilenames, tempdir, outprefix, mode, sampledata):
    ...
    with open(f'{outprefix}.{mode}.var.counts.tsv', 'w') as out:
        ...
        for var in vartocounts:
            if any([x[1] > 0 for x in vartocounts[var]]):  # any modified reads in any sample
...
def quantvarpos():
    ...
    read_vars_to_genome_pos_counts(genenames, tempdir, args.output_prefix, args.mode, sampledata)
