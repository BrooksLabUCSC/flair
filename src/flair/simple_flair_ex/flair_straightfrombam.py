...
from fastercollapse import transcriptomealignandcount
from identify_gene_isoform import bin_search
...
def getrgb(name, strand, junclen):
    if name[:4] == 'ENST': return '3,28,252'
    elif junclen == 0: return "99,99,99"
    elif strand == '+': return "27,158,119"
    else: return "217,95,2"
...
def getexonsfromjuncs(juncs, start, end):
    if len(juncs) == 0:
        ...
        esizes = [juncs[0][0] - start] + [juncs[i + 1][0] - juncs[i][1] for i in range(len(juncs) - 1)] + [end - juncs[-1][1]]
...
class BedRead(object):
    ...
    def generate_from_cigar(self, alignstart, is_reverse, cigartuples, readname, referencename, qualscore, juncDirection):
    ...
    for line in open(gtf):
        if line[0] != '#':
            ...
            if ty == 'transcript' or ty == 'exon':
                ...
                if (this_transcript, this_gene) not in allchromtotranscripttoexons[chrom]: allchromtotranscripttoexons[chrom][(this_transcript, this_gene)] = [None, []]
            ...
            for rstart, rend in chromtoregions[rchrom]:
                if rstart < tstart < rend or rstart < tend < rend:
                    ...
                    if len(juncs) == 0:
                        ...
                        for j in juncs:
                            if j not in regionstoannotdata[thisregion].junctogene: regionstoannotdata[thisregion].junctogene[j] = set()
...
def collapseendgroups(readends, dogetbestends=True):
    ...
    for isoinfo in sortedends:
        ...
        if start - laststart <= args.end_window and end - lastend <= args.end_window:
            ...
            if len(currgroup) > 0:
                if dogetbestends: isoendgroups.append(list(getbestends(currgroup)) + [[x[3] for x in currgroup]])
                else: isoendgroups.append(combinefinalends(currgroup))
            currgroup = [isoinfo]
...
def identifygoodmatchtoannot(args, tempprefix, thischrom, annottranscripttoexons, alltranscripts, genome):
    ...
    if not args.noaligntoannot and len(alltranscripts) > 0:
        transcripttostrand = {}
        with open(tempprefix + '.annotated_transcripts.bed', 'w') as annotbed, open(tempprefix + '.annotated_transcripts.fa', 'w') as annotfa:
            for transcript, gene, strand in alltranscripts:
                ...
                bedline = [thischrom, start, end, transcript + '_' + gene, '.', strand, start, end, '0', len(exons), ','.join([str(x) for x in esizes]), ','.join([str(x) for x in estarts])]
        ...
        with open(tempprefix + '.matchannot.counts.tsv', 'w') as countsout, open(tempprefix + '.matchannot.read.map.txt', 'w') as mapout, open(tempprefix + '.matchannot.bed', 'w') as annotbed:
            pass
    goodaligntoannot = set(goodaligntoannot)
    return goodaligntoannot, firstpasssingleexons, supannottranscripttojuncs

def filtercorrectgroupreads(args, tempprefix, rchrom, rstart, rend, samfile, goodaligntoannot, intervalTree, junctionBoundaryDict):
    sjtoends = {}
...
def filterendsbyredundantandsupport(args, goodendswithsupreads):
    ...
    goodendswithsupreads.sort(key=lambda x: [x[0], x[2] - x[1]], reverse=True)  ##first by weighted score, then by length
...
def processjuncstofirstpassisos(args, tempprefix, thischrom, sjtoends, firstpasssingleexons):
    ...
    with open(tempprefix + '.firstpass.unfiltered.bed', 'w') as isoout:
        for juncs in sjtoends:
            ...
            if juncs == (): groupedgoodendswithsupreads = groupfirstpasssingleexon(goodendswithsupreads)
            else: groupedgoodendswithsupreads = [goodendswithsupreads]
...
def filtersplicediso(args, thisiso, firstpassjunctoname, firstpassunfiltered, junctogene, supannottranscripttojuncs, annottranscripttoexons):
                    ...
                    for i in range(len(otherisoexons)):
                        ...
                        if i == 0 or i == len(otherisoexons) - 1:##is first or last exon of other transcript
                            if firstexon[1] == otherexon[1] or lastexon[0] == otherexon[0]:
                                if firstexon[1] == otherexon[1]: issubset[0] = 1
                                elif lastexon[0] == otherexon[0]: issubset[1] = 1
...
def filtersingleexoniso(args, groupediso, currgroup, firstpassunfiltered):
    ...
    if not iscontained and all(exprcomp): return True
    else: return False
...
def filterfirstpassisos(args, firstpassunfiltered, firstpassjunctoname, firstpasssingleexons, junctogene, supannottranscripttojuncs, annottranscripttoexons):
        ...
        for isoname in firstpassunfiltered:
            ...
            if thisiso.juncs != ():
                if args.filter == 'comprehensive':
                    ...
                    passesfiltering = filtersplicediso(args, thisiso, firstpassjunctoname, firstpassunfiltered, junctogene, supannottranscripttojuncs, annottranscripttoexons)
...
def getgenenamesandwritefirstpass(tempprefix, thischrom, firstpass, juncstotranscript, junctogene, allannotse, genetoannotjuncs, genome):
    with open(tempprefix + '.firstpass.bed', 'w') as isoout, open(tempprefix + '.firstpass.fa', 'w') as seqout:
        ...
        for isoname in firstpass:
            ...
            if thisiso.juncs != () and thisiso.juncs in juncstotranscript:
                ...
                if thisiso.juncs != (): gene_hits = getsplicedisogenehits(thisiso, junctogene, genetoannotjuncs)
                else: gene_hits = getsingleexongenehits(thisiso, allannotse) ##single exon
...
def getbedgtfoutfrominfo(endinfo, chrom, strand, juncs, gene, genome):
    ...
    gtflines = []
    gtflines.append([chrom, 'FLAIR', 'transcript', start+1, end, score, strand, '.', 'gene_id "' + gene + '"; transcript_id "' + iso + '";'])
...
def combineannotnovelwriteout(args, genetojuncstoends, genome):
    with open(args.output + '.isoforms.bed', 'w') as isoout, open(args.output + '.read.map.txt', 'w') as mapout, open(args.output + '.isoforms.gtf', 'w') as gtfout, open(args.output + '.isoforms.fa', 'w') as seqout:
        for gene in genetojuncstoends:
            ...
            for chrom, strand, juncs in genetojuncstoends[gene]:
                ...
                for isoinfo in endslist:
                    ...
                    seqout.write(tseq + '\n')
            gtflines.insert(0, [chrom, 'FLAIR', 'gene', min(tstarts)+1, max(tends), '.', gtflines[0][6], '.', 'gene_id "' + gene + '";'])
            for g in gtflines:
...
def runcollapsebychrom(listofargs):
    ...
    pipettor.run([('samtools', 'view', '-h', args.genomealignedbam, rchrom + ':' + rstart + '-' + rend), ('samtools', 'fasta', '-')],
    ...
    if len(firstpass.keys()) > 0:
        ...
        with open(tempprefix + '.firstpass.fa', 'w') as faout, open(tempprefix + '.firstpass.bed', 'w') as bedout, \
            open(tempprefix + '.novelisos.counts.tsv', 'w') as countsout, open(tempprefix + '.novelisos.read.map.txt', 'w') as mapout:
            pass
    genome.close()
...
def collapse(args):
    ...
    if os.path.getsize(args.genomealignedbam) < 1e+9: #less than 1G
        ...
        pipettor.run([('bedtools', 'bamtobed', '-i', args.genomealignedbam),
                      ('bedPartition', '-minPartitionItems=1000', '-parallel=' + str(args.threads),'/dev/stdin', tempDir + 'regions.bed')])
    ...
    for rchrom, rstart, rend in allregions:
        if rchrom in knownchromosomes:
            ...
            if args.gtf:
                juncstotranscript, junctogene, allannotse, genetoannotjuncs, annottranscripttoexons, allannottranscripts = regionstoannotdata[(rchrom, rstart, rend)].returndata()
    ...
    if not args.noaligntoannot:
        combinetempfilesbysuffix(args, tempprefixes, ['.matchannot.counts.tsv', '.matchannot.read.map.txt', '.matchannot.bed'])
    combinetempfilesbysuffix(args, tempprefixes, ['.firstpass.unfiltered.bed', '.firstpass.bed', '.novelisos.counts.tsv', '.novelisos.read.map.txt'])

    shutil.rmtree(tempDir)

    genetojuncstoends = processdetectedisos(args, args.output + '.matchannot.read.map.txt', args.output + '.matchannot.bed', 'a', {})#, {})
    genetojuncstoends = processdetectedisos(args, args.output + '.novelisos.read.map.txt', args.output + '.firstpass.bed', 'n', genetojuncstoends)#, genetoisoorigins)
